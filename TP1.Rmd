...
library(ISLR2)
library(boot) # pour la validation croisée
library(leaps) # pour la sélection des variables backward
```
...
# On retire les colonnes inutiles, comme Timestamp
df_mod <- df %>%
  dplyr::select(-Timestamp)  # retire Timestamp mais garde IncidentGrade comme cible

# Transformer tous les facteurs en numériques (encodage simple si besoin)
df_mod <- df_mod %>%
  mutate(across(where(is.factor), ~ as.integer(as.factor(.))))

# Modèle de départ : avec toutes les variables prédictives
modele_depart <- lm(IncidentGrade ~ ., data = df_mod)

# Modèle complet : modèle vide
modele_complet <- lm(IncidentGrade ~ 1, data = df_mod)

# Application de la sélection de variables en forward
modele_forward <- stepAIC(
  modele_depart, 
  scope = list(lower = modele_depart, upper = modele_complet),
  direction = "forward",
  trace = TRUE  # Affiche les étapes
)

#Application de la sélection de variables en backward
modele_backward <- stepAIC(
  modele_depart, 
  scope = list(lower = modele_depart, upper = modele_complet),
  direction = "backward",
  trace = TRUE  # Affiche les étapes
...
library(ggplot2)
library(patchwork)
# Extraire les coefficients du modèle sélectionné
coefs <- summary(modele_forward)$coefficients

# Convertir en data frame
coefs_df <- as.data.frame(coefs)
coefs_df$Variable <- rownames(coefs_df)

# Enlever l'intercept pour le graph
coefs_df <- coefs_df %>% filter(Variable != "(Intercept)")

# Graphique des coefficients
p1 <- ggplot(coefs_df, aes(x = reorder(Variable, Estimate), y = Estimate)) +
  geom_col(fill = "steelblue") +
  coord_flip() +  # pour que les variables soient lisibles
  labs(
    title = "Variables sélectionnées par Forward",
    x = "Variables",
...
# Extraire les coefficients du modèle sélectionné
coefs <- summary(modele_backward)$coefficients
...
# Graphique des coefficients
p2 <- ggplot(coefs_df, aes(x = reorder(Variable, Estimate), y = Estimate)) +
  geom_col(fill = "steelblue") +
  coord_flip() +  # pour que les variables soient lisibles
  labs(
    title = "Variables sélectionnées par backward",
    x = "Variables",
    y = "Coefficient estimé"
  ) +
  theme_minimal()
...
#Nous allons remplacer le training_set et le testing_set 80_20
#par la validation croisée, pour ce faire nous devons d'abord définir les modeles
#grâce au backward
#1- selection des variables factiorielles

#df_factor <- df[ , !(names(df) %in% c("Timestamp","IncidentGrade")]
...
regfit.bwd <- regsubsets(IncidentGrade~.,data=df_mod, method='backward')
summary_bwd <- summary(regfit.bwd)

#summary_fwd$adjr2   # R² ajusté
#summary_fwd$bic     # BIC
#summary_fwd$cp      # Cp de Mallows
#summary(regfit.bwd)
which.min(summary(regfit.bwd)$adjr2)
#summary_fwd$outmat  # Modèles sélectionnés (variables incluses)

```
```{r graphe-rss-adjr2-bic-cp}
par(mfrow = c(2, 2))  # 2x2 graphiques
plot(summary_bwd$rss, xlab = "Nombre de variables", ylab = "RSS", type = "l")
plot(summary_bwd$adjr2, xlab = "Nombre de variables", ylab = "R² ajusté", type = "l")
points(which.max(summary_bwd$adjr2), summary_bwd$adjr2[which.max(summary_bwd$adjr2)], col = "red", cex = 2, pch = 20)

plot(summary_bwd$cp, xlab = "Nombre de variables", ylab = "Cp de Mallows", type = "l")
points(which.min(summary_bwd$cp), summary_bwd$cp[which.min(summary_bwd$cp)], col = "red", cex = 2, pch = 20)

plot(summary_bwd$bic, xlab = "Nombre de variables", ylab = "BIC", type = "l")
points(which.min(summary_bwd$bic), summary_bwd$bic[which.min(summary_bwd$bic)], col = "red", cex = 2, pch = 20)

```

Nous avons choisi le critere BIC car nous avons un dataset volumineux et le BIC pénalise fortement ce qui est avantageux dans notre cas.

```{r selection-predicteurs-significatifs}
# doit être remplacé par une méthode comme le forward ou le backward
#which.min(summary_bwd$bic)
coef(regfit.bwd, id = which.min(summary_bwd$bic))
#coef(regfit.fwd, id = which.min(summary_fwd$adjr2))
#coef(regfit.fwd, id = which.min(summary_fwd$cp))

```

```{r recherche-du-meilleur-modele, include=FALSE}
# Extraire les variables sélectionnées
selected_vars <- names(coef(regfit.bwd, which.min(summary_bwd$bic)))[-1]  # Retirer l'intercept

# Créer une formule de régression avec ces variables
Variables <- as.formula(paste("IncidentGrade ~", paste(selected_vars, collapse = " + ")))

# Ajuster le modèle final avec les variables sélectionnées
final_model <- lm(formula, data = df_mod)
...
glm.fits <- glm(Variables, data = df_mod, family = binomial)

cv_result <- cv.glm(df_mod, glm.fits, K = 5)
