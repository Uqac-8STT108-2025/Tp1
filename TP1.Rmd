...
```{r choix-des-predicteurs}

df_predicteurs <- list()
df_predicteurs <- list()
df_train$IncidentGrade <- ifelse(df_train$IncidentGrade == "positif", 1, 0)
df_train_ <- df_train
df_train_[] <- lapply(df_train, function(x) if(is.factor(x)) as.integer(x) else x)
head(df_train_)

for(colonne in colnames(df_train_)[colnames(df_train_) != "IncidentGrade"]){
  model <- glm(IncidentGrade ~ get(colonne), data = df_train_, family = binomial)
  model_summary <- summary(model)
  p_value <- model_summary$coefficients[, 4]
  if ((p_value[2] > 0.00001 & p_value[2] < 0.05)){
    cat('p-value pour', colonne, '= ', p_value[2], "\n")
    df_predicteurs = c(df_predicteurs, colonne)
  }
}
df_predicteurs <- unlist(df_predicteurs)
df_train_prepare <- df_train_[, df_predicteurs]
df_train_prepare$IncidentGrade = df_train$IncidentGrade

model <- glm(IncidentGrade ~., data = df_train_prepare , family = binomial)
summary(model)
...
model <- glm(IncidentGrade ~Timestamp+Usage, data = df_train_prepare , family = binomial)
summary(model)

```
* interpretation :

* C'est donc le moment de calculer l'accurency et interpreter
